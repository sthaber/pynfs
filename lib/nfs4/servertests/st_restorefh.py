...
def testNoFh3(t, env):
    ...
    ops = c.use_obj(env.opts.usefile) + [c.getfh_op(), c.savefh_op()]
    ops += [c.putrootfh_op()]
    ops += [c.restorefh_op(), c.getfh_op()]
    ops += [c.restorefh_op(), c.getfh_op()]
    res = c.compound(ops)
    ...
    fh1 = res.resarray[-7].switch.switch.object
    fh2 = res.resarray[-3].switch.switch.object
    fh3 = res.resarray[-1].switch.switch.object
    if fh1 != fh2:
        t.fail("restored FH does not match saved FH")
    if fh2 != fh3:
        t.fail("second-restored FH does not match restored FH")
