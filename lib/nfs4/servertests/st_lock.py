...
def testFile2(t, env):
    ...
    FLAGS: multiconn lock lockt all
...
def testTimedoutGrabLock(t, env):
    ...
    FLAGS: multiconn lock timed all
    DEPEND: MKFILE MULTICONN
    CODE: LOCK13
    """
    c1 = env.c1
    c1.init_connection()
    c2 = env.get_and_init_secondconn(c1, env.c2)
...
def testLockSeqid(t, env):
    """ Do some things to test out sequence IDs when converting from open to
    lock owner.

    FLAGS: lock open timed all
    DEPEND: LOCK1
    CODE: LOCKSEQID1
    """
    c = env.c1
    ...
    # Create and open second time so we can downgrade
    fh, stateid = c.create_confirm(t.code,
        access=OPEN4_SHARE_ACCESS_WRITE, deny=OPEN4_SHARE_DENY_NONE)
    print "Stateid after open: \n\t%s" % stateid


    fh2, stateid2 = c.open_confirm(t.code, deny=OPEN4_SHARE_DENY_READ)
    print "Stateid after open2: \n\t%s" % stateid2

    # Downgrade to check seqids
    res = c.downgrade_file(t.code, fh2, stateid2,
        OPEN4_SHARE_ACCESS_WRITE, OPEN4_SHARE_DENY_NONE)
    check(res, msg="downgrading")
    stateid = res.stateid
    print "Stateid after downgrade: \n\t%s" % stateid

    # Lock file first time, using open owner -> lock owner
    # Start lockseqid at random number
    lockseqid = 5
    res = c.lock_file(t.code, fh, stateid, 0, 1, WRITE_LT, lockseqid)
    check(res, msg="Locking file %s byte 0, lockseqid=%d" % (t.code, lockseqid))

    lockstateid = res.lockid
    #lockseqid = lockstateid.seqid
    print "LOCK Stateid after lock (lockseqid=%d): \n\t%s" % (lockseqid, lockstateid)
    lockseqid = lockseqid + 1

    # Invalid relocks - these fail against Linux for some reason.
    #res = c.relock_file(lockseqid+1, fh, lockstateid, 1, 1)
    #check(res, NFS4ERR_BAD_SEQID, msg="Locking with lockseqid=%d should fail" %
    #    (lockseqid+1))
    #res = c.relock_file(lockseqid-1, fh, lockstateid, 1, 1)
    #check(res, NFS4ERR_BAD_SEQID, msg="Locking with lockseqid=%d should fail" %
    #    (lockseqid-1))
    ...
    # Valid relock
    res = c.relock_file(lockseqid, fh, lockstateid, 1, 1)
    check(res, msg="Locking file %s byte 1, lockseqid=%d" % (t.code, lockseqid))

    lockstateid = res.lockid
    print "LOCK Stateid after relock (lockseqid=%d): \n\t%s" % (lockseqid, lockstateid)
    lockseqid = lockseqid + 1
    ...
    res = c.close_file(t.code, fh, stateid)
    check(res, msg="failed on close")

    fh, stateid = c.open_confirm(t.code,
        access=OPEN4_SHARE_ACCESS_BOTH, deny=OPEN4_SHARE_DENY_NONE)
    print "Stateid after closing & reopening: \n\t%s" % stateid
    ...
    res = c.close_file(t.code, fh, stateid)
    check(res, msg="failed on close2")
