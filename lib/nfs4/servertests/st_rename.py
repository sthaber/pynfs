...
def testRenameFileLookup(t, env):
    """RENAME of a file after saving the FH and then doing a lookup
    ...
    # Produce a rename with lookup in the middle
    ops = [c.putfh_op(fh_dir)]
    ops += [c.savefh_op()]
    ops += [c.lookup_op(file_name)]
    ops += [c.putfh_op(fh_dir)]
    ops += [c.rename_op(file_name, file_name + '.new')]
    res = c.compound(ops)
    check(res)
