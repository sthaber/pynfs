...
def testNoSfh2(t, env):
    """LINK should fail with NFS4ERR_NOFILEHANDLE if no (sfh) (2)
    ...
    """
    c = env.c1
    ops = c.use_obj(c.homedir) + [c.link_op(t.code)]
    res = c.compound(ops)
    check(res, NFS4ERR_NOFILEHANDLE, "LINK with no <sfh>")
...
def testLinkFileLookup(t, env):
    """LINK of a file after saving the FH and then doing a lookup
    ...
    c.init_connection()
    file_name = 'file'
    c.maketree([t.code, file_name])
    basedir = c.homedir + [t.code]
    file_obj = basedir + [file_name]
    
    # Get the FH for homedir
    fh_file = c.do_getfh(file_obj)
    fh_dir = c.do_getfh(basedir)

    # Produce a link with lookup in the middle
    ops = [c.putfh_op(fh_file)]
    ops += [c.savefh_op()]
    ops += [c.putrootfh_op()]
    for x in basedir:
        ops += [c.lookup_op(x)]
    ops += [c.putfh_op(fh_dir)]
    ops += [c.link_op(file_name + '.link')]
    res = c.compound(ops)
    check(res)
