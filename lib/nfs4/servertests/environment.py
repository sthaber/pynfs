...
# Connection for admin. Used in st_idmap.py
    ...
    # firstconn: This should be the first connection the test uses
    #
    # default_secondconn: Use if you modify a test that already uses a
    # different second connection (that way the test does not change in the
    # non-secondserver case).
    def get_and_init_secondconn(self, firstconn, default_secondconn=None):
        if self.secondconn is None:
            if default_secondconn is None:
                conn2 = firstconn
            else:
                conn2 = default_secondconn
        else:
            conn2 = self.secondconn

        # Init second connection, unless its the same as the first conn
        if conn2 != firstconn:
...
def get_invalid_utf8strings():
    ...
    return ["\xc0\xc1", # starts two multibyte sequences
            ...
            "\xef\xbf\xbf", # Invalid character U+FFFF
            "\xf0\x9f\xbf\xbe", # Invalid character U+1FFFE
            "\xf0\x9f\xbf\xbf", # Invalid character U+1FFFE
            "\xf4\x8f\xbf\xbe", # Invalid character U+10FFFE
            "\xf4\x8f\xbf\xbf", # Invalid character U+10FFFF
            "\xef\xb7\x90", # Invalid character U+FDD0
            "\xef\xb7\xaf", # Invalid character U+FDEF
