...
def _try_mand(env, path):
    # FIRST, try the attributes one at a time.
    _try_mand_oneatatime(env, path)
...
def _try_mand_oneatatime(env, path):
    c = env.c1
    for attr in env.attr_info:
        if attr.mandatory and attr.name != 'rdattr_error':
            mand_bits = [attr.bitnum]
            dict = c.do_getattrdict(path, mand_bits)
            ops = c.use_obj(path) + [c.verify_op(dict)] + c.use_obj(path)
            res = c.compound(ops)
            check(res, msg="Verifying " + attr.name)
...
def _try_unsupported(env, path):
    ...
    for attr in unsupp:
        # XXX ISILON: Skip FS_LOCATIONS. Either pynfs packs it wrong or we
        # decode it wrong on the server.
        if attr.bitnum == FATTR4_FS_LOCATIONS:
            continue
