...
def _compare(t, res, expect, eof=True):
    ...
    if count < len(expect):
        if res.eof:
             t.fail("READ returned EOF after getting %s(%d), expected %s(%d)" %
                   (shorten(res.data), len(res.data), shorten(expect), len(expect)))
        else:
...
def _readloop(c, fh, offset, count, stateid):
    data = ""

    res = c.read_file(fh, offset, count, stateid)

    while res.eof == False and res.status == NFS4_OK and count > 0:
        data += res.data
...
def testLargeCount(t, env):
    ...
    res = _readloop(c, fh, 0, 9000000, stateid)
    check(res, msg="Reading file %s" % t.code)
    _compare(t, res, '\x00'*9000000, False)

    res = _readloop(c, fh, 0, 10000000, stateid)
    check(res, msg="Reading file %s to EOF" % t.code)
    _compare(t, res, '\x00'*10000000, True)
...
def testReadEof(t, env):
    """READ of various sizes to expect what EOF should be set to
    ...
    """
    c = env.c1
    count = c.do_getattr(FATTR4_SIZE, env.opts.usefile)
    eof = False

    for sz in range(0, count*2):
        if sz == count:
            eof = True
        res = c.read_file(env.opts.usefile, offset=0, count=sz)
        check(res, msg="Reading file /%s" % '/'.join(env.opts.usefile))
        _compare(t, res, env.filedata[0:sz], eof)
