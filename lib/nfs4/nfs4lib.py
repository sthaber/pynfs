...
class CBServer(rpc.RPCServer):
    def __init__(self, client, ipv6=False):
        ...
        self.port = 0
        self.client = client
        rpc.RPCServer.__init__(self,prog = self.prog, vers = 1,
                               port = self.port, ipv6 = ipv6)
...
class NFS4Client(rpc.RPCClient, nfs4_ops.NFS4Operations):
    ...
    def init_connection(self, id=None, verifier=None, cb_ident=None, cb_raddr=None):
        """Do setclientid/setclientidconfirm combination"""
        # SETCLIENTID
        res = self.compound([self.setclientid(id, verifier, cb_ident, cb_raddr)])
        check_result(res)
    ...
    def setclientid(self, id=None, verifier=None, cb_ident=None, cb_raddr=None):
    ...
    def supportedACLTypes(self, path=None):
        """Returns bitmask of supported ACL Types"""
        if not path: path = self.homedir
        d = self.do_getattrdict(path, [FATTR4_ACLSUPPORT])
        return d[FATTR4_ACLSUPPORT]
    ...
    def remove_obj(self, parent, file_obj):
        # Set (cfh) to parent path
        ops = self.use_obj(parent)
        # Call remove
        ops += [self.remove_op(file_obj)]
        return self.compound(ops)
